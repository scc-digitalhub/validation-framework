...
class Run:
    ...
    def _parse_profile(self) -> dict:
        ...
        list_columns = ["analysis", "table", "package", "variables"]
        list_var_columns = ["n_distinct", "p_distinct", "is_unique",
        ...
        keys = [k for k in list_columns if k in dict_]
        short_profile = {k: dict_[k] for k in keys}

        if "variables" in short_profile:
            for key in short_profile["variables"]:
                short_profile["variables"][key] = {
                    k: short_profile["variables"][key][k]
                    for k in list_var_columns if k in list_var_columns
                }
